...
export default function renderCountries(dataCountries) {
    ...
    if (quantityCountries > 10) {
        ...
        countryInfo.innerHTML = `
        <div class="large_flag">
            <img src="${currentCountry.flags.svg}" alt="" class="flags" />
            <span class="country">${currentCountry.name.official}</span>
        </div>
        <div>
            <p>Capital: ${currentCountry.capital} </p>
            <p>Population: ${currentCountry.population} </p>
            <p>Languages: ${Object.values(currentCountry.languages)
            .join(', ')} 
                </p>
        </div>`;
